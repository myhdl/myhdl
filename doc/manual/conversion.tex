...
\subsection{Support for RAM memory inference \label{conf-features-ram}}
Certain synthesis tools can map Verilog memories to memory
structures. To support this interesting feature, the Verilog convertor
maps lists of signals to Verilog memories.

\subsection{Support for ROM memory inference \label{conf-features-rom}}
Some synthesis tools can infer a ROM memory
...
In contrast, Verilog uses an unsigned representation by default and a
signed representation when it is explicitly declared. When the two are
mixed in an expression, the operation is performed unsigned, which
typically leads to unexpected results.
