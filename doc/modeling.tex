...
\begin{quote}
\em
The section on high level modeling should become an exciting part of
...
we don't want to store the signal object, but its current value. (In
many cases, \myhdl\ can use a signal's value automatically when there
is no ambiguity: this happens for example whenever a signal is used in
expressions. When in doubt, you can always use the \code{val}
