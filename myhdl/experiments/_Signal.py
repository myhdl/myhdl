...
class Signal(object):
    ...
    def _listener(self, sig, *args, **kwargs):
        debug = 0
    ...
    def _driver(self, sig, *args, **kwargs):
        debug = 0
        if kwargs.has_key('debug'):
            debug = 1
        if len(args) == 1:
            index = args[0]
            oldval = 0
            #oldval = self.val[index]
#            sig.next = newval
            while 1:
                newval = self.val[index]
                if newval != oldval:
                #if 1:
                    sig.next = newval
                    oldval = newval
## ##                 print self.val[index]
                ...
                if debug:
                    print "DR %s: %s" % (now(), self.val[index])
                yield self
        elif len(args) == 2:
            high, low = args
            if low is None:
    ...
    def __call__(self, *args, **kwargs):
        if len(args) == 1:
            index = args[0]
            sig = Signal(self.val[index])
        elif len(args) == 2:
            high, low = args
            if low is None:
                low = 0
            if high is None:
                sig = Signal(self.val[:low])
            else:
                sig = Signal(self.val[high:low])
        else:
            raise TypeError
        listener = self._listener(sig, *args, **kwargs)
        sig._eventWaiters.append(_Waiter(listener))
        driver = self._driver(sig, *args, **kwargs)
